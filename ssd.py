...
class SSD:
    def __init__(self,use_device):
        ...
        self.model_xml = "./weights/ssdlite.xml"
        self.model_bin = "./weights/ssdlite.bin"
        ...
        while cap.isOpened():
            ...
            if self.exec_net.requests[0].wait(-1) == 0:
                ...
                for out_blob in result.values():
                    ...
                    results = self.bbox_util.detection_out(out_blob)
                    print("長さ:",len(results[0]))
                    if len(results[0]) != 0:
                        print(results)
                        ...
                        boxes = ssd_correct_boxes(top_ymin,top_xmin,top_ymax,top_xmax,np.array([320,320]),np.array([cap_h,cap_w]))
                        for i,c in enumerate(top_label_indices):
                            ...
                            bottom = min(int(cap_h), np.floor(bottom + 0.5).astype('int32'))
                            right = min(int(cap_w), np.floor(right + 0.5).astype('int32'))
                            print(top,left,bottom,right)
                            x_center = int((right - left))
                            y_center = int((bottom - top))
                            frame = cv2.rectangle(frame,(left,top),(right,bottom),(0,255,0),3)
                            frame = cv2.putText(frame,idx2label[int(c)-1],(left-3,top-3), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255),3)
